...
- Status: Active
- Review level: L9
- Target: 在第一方 Runtime Capsule 边界下，通过六平台原生 CI、候选聚合、manifest/checksum、精确 Tag/Draft 恢复、Homebrew Tap 同步和 Pages 身份门禁交付公开 Preview。

### 当前事实

- 当前活动应用版本是 `0.1.0+3`；Node 与 Protocol 为私有 `0.2.0-dev.0`。
- 活动 Profile 是 `independent-first-party-preview-v1`，且 `publicationEnabled: true`；只有显式 dispatch 的 Preview publish 才能创建不可变 Tag、Release、Tap Cask 和 release-bound Pages。保留的 `independent-six-platform-development-v1` 仍用于 publication-disabled 源码资格验证。
...
3. 聚合 Preview workflow 在目标原生 runner 构建 Release 形态；桌面先构建 Capsule，再安装进 App/Bundle，运行平台 verifier 与 bundled-app E2E，macOS 还必须证明 Universal，最后 stage 标准资产。
...
5. `qualify` 可以上传同一 commit 的短期 Actions bundle。`publish` 必须先通过 `--require-publication`、Tap credential 和精确远端身份门禁；Release、Tap 和 Pages 的 mutation 只在所有资格步骤通过后执行。
6. 继续使用 `DEC-016` 的 annotated Tag、精确 commit、原 qualification run、Draft readback、同名字节不覆盖、单个 failed starter 有界修复和 publish-last 机制；公开 Preview 追加公开资产回读、Cask 精确生成和 Homebrew 安装 smoke。
7. Pages 的 main push 只部署 source-only 产品站；Release dispatch 必须绑定 annotated Tag/commit，验证公开 Release 和 Tap Cask 完全匹配后才部署包含 Homebrew 命令的 Preview 页面。
8. Homebrew 工具不读取移动 ref 或占位 checksum；必须显式提供公开 Tag、commit、asset、SHA-256 和 Universal macOS 资格证据。发布 workflow 通过 `HOMEBREW_TAP_TOKEN` 更新同一个 Cask，并在远端回读后执行 macOS 安装验证。
...
- `VER-PI-024` 记录六平台 Preview qualification、公开 Release、下载回读、Tap Cask 和 release-bound Pages evidence；未完成远端证据前不得声称 Homebrew 可用。
- `VER-PI-026` 只有在公开资产、Tap 回读、干净 macOS 安装、Runtime Capsule 和卸载 evidence 完成后才能 PASS。
- 后续 Preview Release 沿用同一 Homebrew 命令，自动更新 Cask version、URL 和 checksum；历史 `v0.0.2`/`v0.0.3` 身份保持不可变。
