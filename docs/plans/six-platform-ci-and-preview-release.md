...
1. 使用共享 composite Action 固定 Dart/FVM/Flutter，并关闭冲突的 Dart problem matcher；CI 同时运行 Flutter、Protocol、Pi Node、Release 工具、Astro 与六平台构建。
...
4. 每个 stage evidence 记录真实 `hostRuntimeIncluded` 与验证方法；聚合 manifest 固定完整 commit、版本、Flutter、平台、架构、角色、签名、runtime baseline、大小与 SHA-256。
