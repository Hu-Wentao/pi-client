...
- These statuses describe source implementation only. They do not activate `REQ-PI-006`, `REQ-PI-009`, or `REQ-PI-013`, publish Protocol 1.0, prove a provider-backed production prompt, bundle a Capsule into supported desktop packages, qualify project-trust UX, or establish a supported stable release.
...
- The runtime Capsule builder verifies a host-targeted Node/Pi Node payload, manifest, checksums, and forbidden-artifact scan, but no supported stable desktop application currently bundles and qualifies that Capsule. Windows/Linux packaging, signed/notarized stable distribution, and the first supported independent release remain incomplete.
- README and the ordinary main Landing Page describe current source-only development; the exact release-bound Landing Page exposes only the verified Homebrew Preview CTA. The public `v0.0.2` and transitional `v0.0.3` Releases remain immutable historical evidence outside current delivery surfaces.
