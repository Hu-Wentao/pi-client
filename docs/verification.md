...
- Evidence: The deployed source at `e2110c0c03c03d9b4de8c7bfc62c303de00a375b` passed `bun run brand`, Astro check/build, and built-site validation for English and Simplified Chinese routes with canonical `pi.wyattcoder.top` metadata, root-relative brand assets, six platform names, verified platform-role wording, GitHub/contribution CTAs, active-development disclosure, and no client JavaScript. Pages run `33365568081` bound that exact SHA and completed source, build, Release-asset admission, and deploy Jobs successfully. Pages readback reports `cname=pi.wyattcoder.top`, a verified domain, approved certificate, and `https_enforced=true`; production checks observed HTTP `301`, `200` for `/`, `/zh-cn/`, the product mark and social card, `404` for the retired Workspace screenshot, correct canonicals, required six-platform content, and no legacy runtime, archived version/download, screenshot, or `/pi-client/` asset references. Current source validation additionally derives the active Preview URL, Tag and Homebrew command from the Release contract and rejects all three from rendered pages; 55 focused Node tests, Flutter analysis, all 34 Flutter tests, Astro check/build and built-site validation pass locally.
- Scope limit: The existing production deployment proves the independent product content before the Homebrew release source change. The annotated-Tag Pages run for `v0.0.3` must reverify the expanded active-Preview/Homebrew exclusion; representative visual regression, Lighthouse, VoiceOver, and 200% zoom acceptance remain separate manual quality evidence.
...
- Planned evidence: Generate `Casks/pi-client.rb` from the public `v0.0.3` macOS ZIP SHA-256; verify exact Cask bytes and passing style; record that official-new-cask audit rejects the intentionally unsigned Prerelease rather than treating it as the custom-Tap success gate; install from the public Tap into `/Applications`; verify `io.github.huwentao.piClient`, version `0.0.3`, build `3`, arm64+x86_64 application/runtime binaries, preserved quarantine and expected Gatekeeper rejection; then uninstall through Homebrew.
- Gap: The source contract is not yet merged, `v0.0.3` is not yet published, the Tap does not yet exist, and no public one-line installation has been observed.
