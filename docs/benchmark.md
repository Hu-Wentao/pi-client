...
- Independent correction: Pi Client desktop artifacts are Agent-host-capable and must contain a verified first-party Runtime Capsule; Android, iOS, Web JavaScript, and WebAssembly remain connect-only. The current `0.1.0+3` Profile is a publication-enabled Preview profile, but qualification alone is not a release claim.
