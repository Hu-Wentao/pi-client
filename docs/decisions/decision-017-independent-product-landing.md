...
- 推荐：选项 C；它满足独立产品身份，同时避免把尚未公开发布的第一方运行时描述为可下载、受支持的交付。
- 选择：选项 C
- 同意影响：规范域名继续为 `https://pi.wyattcoder.top/`；页面不渲染旧下载、旧 Workspace 截图、旧安装说明或遗留适配器名称；主 CTA 为 GitHub 和贡献指南；页面可以说明源码中的第一方 Local Direct 与 Runtime Capsule，但必须显著说明当前没有受支持的公开二进制入口。
