...
- 选择：选项 C
- 当时同意影响：`docs/benchmark.md` 完整分类可见能力、Native-adapted 项和排除项；当时的 `docs/requirements.md` 是实现与验收的语义权威；外部快照变化不自动扩大当时规划的 `1.0.0` 范围。该影响现只解释历史材料，不约束当前 `1.0` 范围。
...
- 历史计划关系：`PLAN-PI-004` 当时拥有 P0-P11 至 `1.0.0` 的实施顺序；`PLAN-PI-002` 作为 Friday Workspace 并行轨道；`PLAN-PI-001` 继续保持 Superseded，不因本决策复活。上述关系不使已归档 parity material 进入当前 `docs/plans/pi-client-1.0.md`。
