...
test("all external Actions remain pinned to full commit SHAs", async () => {
  const files = [
    ...(await yamlFiles(workflowsRoot)),
    ...(await yamlFiles(actionsRoot)),
  ];
...
test("composite Flutter setup carries the Dart problem-matcher fix across hosts", async () => {
  const source = await readFile(
    resolve(actionsRoot, "setup-flutter/action.yml"),
    "utf8",
  );
...
test("aggregated development qualification is Capsule-aware and publication-disabled", async () => {
  ...
  for (const required of [
    ...
    assert.ok(
      ...
      `release-preview.yml must contain ${required}`,
    );
  }
  for (const forbidden of [
    ...
    assert.ok(
      ...
      `release-preview.yml must not contain ${forbidden}`,
    );
  }
});
...
test("Capsule-aware macOS and Windows/Linux candidate workflows remain available", async () => {
  ...
  for (const required of [
    ...
    assert.ok(
      macos.includes(required),
      `release-macos.yml must contain ${required}`,
    );
  }
  assert.ok(
    macos.indexOf(
  ...
  for (const required of [
    ...
    assert.ok(
      desktop.includes(required),
      `release-desktop-candidates.yml must contain ${required}`,
    );
  }
});
...
test("stable desktop admission fails closed when signing credentials are absent", async () => {
  ...
  await assert.rejects(
    () =>
      execFileAsync(
        process.execPath,
        [script, "--platform", "windows", "--channel", "stable"],
        { env },
      ),
    /signing/i,
  );
  await assert.rejects(
    () =>
      execFileAsync(
        process.execPath,
        [script, "--platform", "linux", "--channel", "stable"],
        { env },
      ),
    /signing/i,
  );
...
test("site validation rejects unpublished downloads, Homebrew commands, and stale screenshots", async () => {
  ...
  for (const required of [
    ...
    assert.ok(
      source.includes(required),
      `site validation must contain ${required}`,
    );
  }
});
